...
(function () {
  ...
  document.querySelectorAll('.ic').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      fetch('wsu-eit-icon-' + name + '.svg')
        .then(function (r) { if (!r.ok) throw new Error('nofile'); return r.text(); },
              function () { throw new Error('nofile'); })
        .then(function (text) {
          if (typeof text !== 'string') throw new Error('nofile');
          /* inner markup of the file's root <svg>, minus fixed ink/size so
             the pasted icon inherits text color and em-sizing */
          var open = text.indexOf('>', text.indexOf('<svg')) + 1;
          var inner = text.slice(open, text.lastIndexOf('</svg>'))
            .replace(/<\?xml[^>]*>|<!--[\s\S]*?-->|<title>[\s\S]*?<\/title>/g, '')
            .replace(/\s+(?:fill|stroke)="[^"]*"/g, '')
            .replace(/\s+/g, ' ').trim();
          var vb = (text.match(/viewBox="([^"]+)"/) || [, btn.getAttribute('data-vb')])[1];
          var snippet = '<svg class="wsu-eit-icon" viewBox="' + vb + '" aria-hidden="true" focusable="false">' + inner + '</svg>';
          /* clipboard failure is NOT a missing file — report it honestly
             (iframes/permissions can block writes even when the file is fine) */
          return navigator.clipboard.writeText(snippet).then(
            function () { flash('Copied!'); },
            function () { flash('Copy blocked'); });
        })
        .catch(function (e) { flash(e && e.message === 'nofile' ? 'File missing!' : 'Copy failed'); });
    });
  });
})();
...
(function () {
  ...
  function prettyPrint(src) {
    ...
    for (i = 0; i < tokens.length; i++) {
      t = tokens[i].trim();
      if (!t) continue;
      var startsClose = /^<\//.test(t);
      if (startsClose) depth = Math.max(0, depth - 1);
      out.push(new Array(depth + 1).join('  ') + t);
      /* net nesting change within this token (opens minus closes, voids and
         self-closing excluded); add back the dedent we already applied */
      var opens = (t.match(/<([a-z][a-z0-9-]*)/gi) || []).filter(function (m) {
        return !VOID[m.slice(1).toLowerCase()];
      }).length;
      var closes = (t.match(/<\/[a-z][a-z0-9-]*/gi) || []).length;
      var selfClosed = (t.match(/\/>/g) || []).length;
      depth = Math.max(0, depth + opens - selfClosed - closes + (startsClose ? 1 : 0));
    }
    return out.join('\n');
  }
  ...
  document.querySelectorAll('pre.snip').forEach(function (pre) {
    ...
    pre._base = code.textContent.trim().replace(/\n\s*/g, '');  /* pristine markup */
    ...
    bar.innerHTML = '<button type="button" class="snip-view" data-v="pretty" aria-pressed="true">Pretty</button>'
...
(function () {
  ...
  var info = Array.prototype.map.call(document.querySelectorAll('.pat'), function (p) {
    var demo = p.querySelector('.pat-demo');
    ...
    return { el: p, classes: uniq, parent: p.parentNode, next: p.nextSibling };
  });
  ...
  function setMode(mode) {
    if (mode === 'class') {
      ...
      /* restore in REVERSE document order: a pattern's remembered next-
         sibling is often another pattern; walking backwards guarantees that
         sibling is already back in place before we insert ahead of it.
         (Forward order threw NotFoundError on the first such pair and
         locked the toggle.) */
      for (var i = info.length - 1; i >= 0; i--) {
        var it = info[i];
        if (it.next && it.next.parentNode === it.parent) {
          it.parent.insertBefore(it.el, it.next);
        } else {
          it.parent.appendChild(it.el); /* never strand a card */
        }
      }
      byClass.hidden = true;
    }
    document.getElementById('nav-chapters').hidden = (mode === 'class');
  ...
  function fixGroups() {
    document.querySelectorAll('.clsgrp').forEach(function (g) {

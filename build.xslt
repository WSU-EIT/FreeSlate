...
<!--
  ...
  build.xslt - the Slate global page template. Third-party pieces retained
  by necessity: the Technolutions framework contract, the GTM loader scripts,
  Google Fonts / wsu-icons CDN, and the official WSU cougar mark + brand URLs.

  Ships with the kit's deploy set, all in /shared/: build-fonts.css (fonts),
  build.css (all styling), wsu-eit-extras.css (opt-in components),
  wsu-eit-a11y.js (accessibility repairs) and wsu-eit-extras.js (opt-in
  behaviors). The Branding Editor's Publish writes the build.* trio every
  time, so Publish IS the deploy.
  ...
  Slate renders each external page to an XHTML document, then runs this file
  as an XSLT 1.0 transform over it. Two jobs:
    1. The match="/" template builds the page chrome (head, asset links,
       topbar, footer) and decides WHERE Slate's content lands.
    2. The identity template at the bottom copies everything else through
       unchanged.
  Two lines are pure Slate framework contract, do not remove:
    · <template path="/shared/base.xslt" xmlns="http://technolutions.com/
      framework" /> pulls in Slate's framework base template.
    · <xsl:apply-templates select="xhtml:html/xhtml:body/node()" /> (inside
      main.wsu-eit-zone) is the HAND-OFF POINT where Slate's rendered body
      lands in our frame. We own the container; Slate owns the contents -
      which is exactly why every kit rule can scope to .wsu-eit/.wsu-eit-zone.
  The head pass-through (xhtml:html/xhtml:head/node()) preserves Slate's
  framework CSS, per-form Edit Styles, and the page <title>.

  CONDITIONAL BRANDING (not used yet)
  To branch chrome by URL path, query param, or application round, declare
  the framework namespace on the root <xsl:stylesheet> (xmlns:fw="http://
  technolutions.com/framework") and use fw:path(), fw:query('param'),
  fw:https(), fw:config(key), or fw:year(). Prefer xsl:choose / contains()
  over xsl:if (the KB warns plain If can leave branding loopholes). Today we
  declare the framework namespace only on the inline <template> element,
  since no fw: conditional functions are in use. See SLATE-COOKBOOK.md.
-->
  ...
  <xsl:template match="/">
    <html xmlns="http://www.w3.org/1999/xhtml">
      ...
      <head>
        ...
        <link href="/shared/build-fonts.css?v=20260622" rel="stylesheet" />
        <!-- build.css — the whole styling layer: design tokens, page frame,
        ...
        <!-- Opt-in editor components (reel/carousel, FAQ accordion, steps,
             ...
             extras, nothing else changes. -->
        <link href="/shared/wsu-eit-extras.css?v=20260622" rel="stylesheet" />
        <!-- THE ONE JavaScript exception (dept. policy: CSS-first).
             wsu-eit-a11y.js repairs what stylesheets cannot: Slate emits
        ...
        <script src="/shared/wsu-eit-a11y.js?v=20260622" defer="defer"><xsl:text> </xsl:text></script>
        ...
        <script src="/shared/wsu-eit-extras.js?v=20260622" defer="defer"><xsl:text> </xsl:text></script>
